...
export default function VideoTrimmerTool() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError({ title: 'Video engine failed to load', why: err.message, action: 'Try Chrome or Edge.' });
        setState('error');
      }
    };
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    if (!f.type.startsWith('video/')) {
      setError({ title: 'Unsupported format', why: (f.type || f.name), action: 'Upload MP4, MOV, or WebM.' });
      return;
    }
  ...
  const exportVideo = useCallback(async () => {
    ...
    try {
      ...
      setError({ title: 'Export failed', why: msg, action: 'Try a shorter clip.' });
      setState('error');
    }
  ...
  const handleDownload = useCallback(() => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = (file?.name?.replace(/\.[^.]+$/, '') || 'clip') + '-trimmed.mp4'; a.click();
  ...
  if (state === 'loading-engine') {
    return <div style={{ padding: 'var(--space-8) 0' }}><ProgressBar value={null} label={'Loading video engine (~25MB, first time only)...'} /></div>;
  }

  if (state === 'processing') {
    return <div style={{ padding: 'var(--space-8) 0' }}><ProgressBar value={null} label='Trimming video...' /></div>;
  }

  if (state === 'done' && resultBlob) {
    return (
      <div className='animate-in' style={{ maxWidth: 520, margin: '0 auto' }}>
        <div style={{ marginBottom: 'var(--space-4)' }}><div className='section-label'>Exported MP4</div>
          <video src={URL.createObjectURL(resultBlob)} controls style={{ width: '100%', maxHeight: 400, borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }} />
        </div>
        <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'center' }}>
          <ExportButton onClick={handleDownload} format='MP4' />
          <button className='btn btn-secondary btn-lg' onClick={handleReset}>Trim another</button>
        </div>
      </div>
    );
  }
    ...
    <div>
      {error && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}><button className='btn btn-sm btn-secondary' onClick={handleReset}>Process another video</button></div>
        </div>
      )}
      {!videoUrl ? (
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          <UploadZone onFiles={handleFiles} accept='video/mp4,video/quicktime,video/webm' label='Drop a video here' hint='MP4, MOV, or WebM - up to 500MB' meta='Your video never leaves your device.' maxSizeMB={SOFT_MB} hardMaxSizeMB={HARD_MB} />
        </div>
      ) : (
        <div>
          <div className='tool-workspace'>
            <div><video ref={videoRef} src={videoUrl} controls style={{ width: '100%', maxHeight: 400, borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }} /></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              ...
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>Duration: {(trimEnd - trimStart).toFixed(1)}s</div>
              <ExportButton onClick={exportVideo} label='Export as MP4' disabled={trimStart >= trimEnd || state === 'processing'} />
            </div>
          </div>
        </div>
      )}
